...
const paperPath = path.join(rootDir, "毕业设计AI稿.md");
const featureIndexPath = path.join(rootDir, "docs", "论文析出", "功能技术文档.md");
const featureDocsDir = path.join(rootDir, "docs", "论文析出", "按功能拆分");
const outputDir = path.join(rootDir, "src", "showcase", "data");
...
function main() {
  ...
  const paperMarkdown = readText(paperPath);
  const paperSections = parsePaperSections(paperMarkdown);
  ...
  const data = {
    meta: {
      ...
      generator: "scripts/generate-showcase-data.mjs",
      featureCount: features.length,
      paperSourcePath: "毕业设计AI稿.md",
      featureIndexPath: "docs/论文析出/功能技术文档.md",
      featureDocsDir: "docs/论文析出/按功能拆分",
    },
    paper: {
      title: "基于Tauri的Markdown编辑器",
      sourcePath: "毕业设计AI稿.md",
  ...
  console.log(`Generated ${toRootRelative(outputPath)} with ${features.length} features.`);
}
